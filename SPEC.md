...
Мастеро — строительный маркетплейс, соединяющий клиентов с мастерами-ремонтниками и перевозчиками. Клиент создаёт заявку (тип работы, площадь, адрес), мастера откликаются с ценой и сроками, клиент выбирает исполнителя. Стороны общаются напрямую через встроенный чат. Отдельный тип заявок — перевозка грузов.
...
### Авторизация
- Регистрация: email + пароль + выбор роли (клиент / мастер / перевозчик)
...
| role | enum('client','master','carrier') NOT NULL |
| name | text NOT NULL |
| phone | text |
| city_district | text |
...
- [ ] Роль строительного магазина — новая роль пользователя
...
| Роль строительного магазина | 🔜 следующий шаг |
| Онлайн-оплата | 🔜 после магазина |
...
- **Оплата:** ЮKassa / Stripe, поле `payment_status` в `orders` и `transport_orders`, эскроу-флоу
