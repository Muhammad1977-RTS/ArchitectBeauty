...
@Component({
  selector: 'app-master-transport-order-detail',
  imports: [RouterLink],
  templateUrl: './transport-order-detail.html',
})
export class MasterTransportOrderDetailComponent implements OnInit, OnDestroy {
  ...
  private scrollChat() {
    setTimeout(() => {
      const el = document.getElementById('master-transport-chat');
  ...
  async selectCarrier(response: TransportResponse) {
    ...
    if (ok) {
      ...
      await this.loadChat(order.id, response.carrier_id);
    }
  ...
  statusLabel(status: string): string {
    const map: Record<string, string> = {
      new: 'Новая', carrier_selected: 'Перевозчик выбран',
